...
bool Application::selectTerrain(bool& valleyTerrain, MacroTerrain::Landform& landform,
    ...
    // 1v1 Match works on both terrain modes: advanced terrain always
    // computes a second spawn as part of its own navigation result, and
    // legacy terrain now runs its own seed search at load time (see
    // TerrainSelection::selectLegacy) specifically when match mode is
    // requested. Either path fails loudly (an exception during loading) if
    // no second spawn can be found, rather than silently leaving
    // hasOpponent_ false -- see hasOpponent_'s own comment.
    ...
    constexpr int kControlCount = 9;
    std::array<Rect, kControlCount> controls{};
    for (int i = 0; i < 2; ++i) controls[i] = {232, 118.f + i * 56, 560, 48};
    // LANDFORM/SEED row sits right below the terrain-mode buttons above.
    controls[2] = {232, 250, 272, 40};
    controls[3] = {512, 250, 168, 40};
    controls[4] = {688, 250, 104, 40};
    // MATCH MODE buttons: same shape as the terrain-mode buttons above,
    // stacked below the LANDFORM/SEED row.
    for (int i = 0; i < 2; ++i) controls[5 + i] = {232, 358.f + i * 56, 560, 48};
    controls[7] = {232, 506, 272, 44};
    controls[8] = {688, 506, 104, 44};
    // The menu opens on the fast/original terrain (no erosion simulation,
    // near-instant load) and 1v1 Match, regardless of the CLI flags (which
    // only automation runs set anyway). Advanced terrain and freeroam are
    // each one click away.
    int selected = 0, focus = 7, form = 0, matchModeSelected = 1;
    ...
    while (!glfwWindowShouldClose(window_)) {
        ...
        if (activated == 5) matchModeSelected = 0;
        if (activated == 6) matchModeSelected = 1;
        if (focus == 3) {
        ...
        if (activated == 8) { glfwSetWindowShouldClose(window_, GLFW_TRUE); break; }
        if (activated == 7 && validSeed) {
            ...
            matchEnabled = matchModeSelected == 1;
        ...
        presentLoadingProgress(0, [&] {
            ...
            text("MATCH MODE", 232, 340, 1.5f, white);
            for (int i = 0; i < kControlCount; ++i) {
                bool disabled = (i == 2 && selected == 0) || (i == 7 && !validSeed);
                Rect r = controls[i];
                ...
                if (i == 5 || i == 6) {
                    ...
                    text(matchModes[m].description,r.x+18,r.y+27,1.1f,muted);
                }
            }
            text("LANDFORM",232,232,1.1f,muted);
            text(selected == 0 ? "NOT USED" : formNames[form],246,263,1.4f,selected == 0 ? muted : white);
            text("SEED",512,232,1.1f,muted);
            if (focus == 3 && selectSeed) quad({516,256,152,28},{.14f,.28f,.27f});
            text(seed.empty() ? "-" : seed,520,263,1.4f,white);
            text("RANDOM",712,264,1.3f,white);
            text(validSeed ? "TAB / ARROWS: FOCUS   ENTER: ACTIVATE   ESC: QUIT" : "ENTER A SEED FROM 0 TO 4294967295",232,300,1.2f,validSeed ? muted : warn);
